...
function Output($name='', $dest='')
{
	...
	switch($dest)
	{
		case 'I':
			...
			if(ob_get_length())
				$this->Error('Some data has already been output, can\'t send PDF file1');
			if(php_sapi_name()!='cli')
			{
				//We send to a browser
				header('Content-Type: application/pdf');
				if(headers_sent())
					$this->Error('Some data has already been output, can\'t send PDF file2');
		...
		case 'D':
			//Download file
			if(ob_get_length())
				$this->Error('Some data has already been output, can\'t send PDF file3');
			header('Content-Type: application/x-download');
			if(headers_sent())
				$this->Error('Some data has already been output, can\'t send PDF file4');
			header('Content-Length: '.strlen($this->buffer));
